 ...
 * single `min(MAX_WAIT, budget / 4)` quartering (see LESSONS.md B-028). The
...
class ReachabilityGateTest {
    ...
    fun fiveMinuteMacOSBudgetWaitsFullMaxWaitEndToEnd() =
        runTest {
            // Regression for the double-quartering bug (B-028). macOS hands the
    ...
    fun infiniteBudgetClampsToMaxWaitEndToEnd() =
        runTest {
            ...
            // == 5s`. Before B-028 the JVM path double-quartered this INFINITE
