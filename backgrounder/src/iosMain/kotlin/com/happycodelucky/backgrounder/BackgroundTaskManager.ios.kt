 ...
 *
 * Optional: `BackgroundTaskManager.shared` builds itself on first access using
 ...
 *   [BackgroundTaskManager.start] time and reported with a Kermit error if missing.
 *
 ...
 *   [BackgroundTaskManager.start] before the launch method returns.
 *
...
): BackgroundTaskManager = IOSBackgrounderBuilder.build(tickIdentifier, eventListener)

/**
 * The tick identifier `BackgroundTaskManager.shared` uses when the app never called
...
public fun BackgroundTaskManager.Companion.defaultTickIdentifier(): String =
...
internal actual fun createDefaultBackgrounder(): BackgroundTaskManager =
    BackgroundTaskManager.create(tickIdentifier = BackgroundTaskManager.defaultTickIdentifier())
